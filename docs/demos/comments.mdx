...
Since all annotated nodes contain the `data-id` attribute, you can select these nodes with the relevant document `querySelector`. Note that the `id` attribute is conveniently and _uniquely_ provided for annotated nodes that have the `data-start="true"` attribute values. This demo uses the `data-start`, `data-end` attributes to determine how comment nodes are placed.
