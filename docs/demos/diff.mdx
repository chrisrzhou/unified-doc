...
An interesting application of this demo is the ability to visually diff only the text content (ignoring non-textual changes e.g. HTML elements). To explore this use case, try the following:

- Apply the same _HTML_ content in both text editor.
...
The demo uses the [`diff`](https://github.com/kpdecker/jsdiff) package to diff two string content with the `diffChars` method. Upon receiving the diffed parts, we can loop over it to construct the `diffContent` and easily keep track and assemble `annotations` in a single loop! We apply very simple CSS to style removed/added parts.
