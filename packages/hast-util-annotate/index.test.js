...
describe('hast-util-annotate.js', () => {
	describe('getTextSegments', () => {
		...
		it('should not return segment if annotate strictly left of position', () => {
			expect(getTextSegments(node, { startOffset: 0, endOffset: 40 })).toEqual(
				null,
			);
		});
		it('should not return segment if annotate strictly right of position', () => {
			expect(
				getTextSegments(node, { startOffset: 100, endOffset: 200 }),
			).toEqual(null);
		});
		it('should return segment if annotate is exactly the same as position', () => {
			expect(
				getTextSegments(node, { startOffset: 40, endOffset: 100 }),
			).toEqual(['', 'abcdefghijklmnopqrstuvwxyz', '']);
		});
		it('should return segment if annotate is a contained within position', () => {
			expect(
				getTextSegments(node, { startOffset: 44, endOffset: 54 }),
			).toEqual(['abcd', 'efghijklmn', 'opqrstuvwxyz']);
		});
		it('should return segment if position is contained within annotate', () => {
			expect(
				getTextSegments(node, { startOffset: 0, endOffset: 200 }),
			).toEqual(['', 'abcdefghijklmnopqrstuvwxyz', '']);
		});
		it('should return segment if annotate is contained left of position', () => {
		...
		it('should return segment if annotate is contained right of position', () => {
			expect(
